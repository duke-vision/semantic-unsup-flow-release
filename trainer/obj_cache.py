...
class ObjectCache:
    def __init__(self, class_name="car", cache_size=100):
    ...
    def init_cache(self, img_size):
        self._obj_mask_cache = torch.zeros((self.cache_size, 1, *img_size), dtype=torch.float32)
        self._img_cache = torch.zeros((self.cache_size, 3, *img_size), dtype=torch.float32)
        self._sem_cache = torch.zeros((self.cache_size, num_class, *img_size), dtype=torch.float32)
        self._motion_cache = torch.zeros((self.cache_size, 2), dtype=torch.float32)        
        return
    
    
    def pop(self, B=4, with_aug=True):  # we do not remove objects after popping
        ...
        sem = self._sem_cache[idx]
        motion = self._motion_cache[idx]
        
        if with_aug:
            rand_scale = torch.rand(B) * 0.7 + 0.8  # randomly rescale motion by 0.8-1.5 times
            rand_scale *= (-1) ** (torch.rand(B) > 0.5).float()  # randomly reverse motion                    
            motion = motion * rand_scale[:, None]
            
            ''' 
            # do not flip here because flipping the 19-channel sem map is very time consuming
            flip_flag = torch.rand(B) > 0.5  # randomly horitontal-flip obj mask
            img[flip_flag] = img[flip_flag].flip(dims=[3])
            sem_class[flip_flag] = sem[flip_flag].flip(dims=[3])
            obj_mask[flip_flag] = obj_mask[flip_flag].flip(dims=[3])
            motion[flip_flag, 0] *= -1 
            '''
  
        return obj_mask, img, sem, motion

        
    def push(self, obj_mask, img, sem, motion):
        '''
        obj_mask: [B, 1, H, W]
        img: [B, 3, H, W]
        sem: [B, 19, H, W]
        ...
        if self.count <= self.cache_size - B: # many spaces
            ...
            self._sem_cache[self.count:(self.count+B)] = sem
            ...
            self._sem_cache[self.count:] = sem[:space]
            ...
            self._sem_cache[overwrite_idx] = sem[space:]
            ...
            self._sem_cache[overwrite_idx] = sem
